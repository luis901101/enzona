...
class CustomOauth2Client extends http.BaseClient {
  ...
  CustomOauth2Client(this._credentials,
      {this.identifier,
      this.secret,
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    if (credentials.isExpired) {
      if (!canRefresh) throw ExpirationException(credentials);
  ...
  Future<CustomOauth2Client> refreshCredentials([List<String>? newScopes]) async {
    if (!canRefresh) {
